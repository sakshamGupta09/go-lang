...
- Recursively navigate through all the folders and their content starting from the root node provided.
...
- If directory has content inside, it will throw an error.

```go
func main() {
	err := os.Remove("root/root-1.txt")
...
- Remove a director along with all it's content
