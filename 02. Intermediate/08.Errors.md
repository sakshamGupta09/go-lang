...
- Custom errors are just a type that implements `Error` methods.
- To tell Go that this is not another struct but an error type, we need to implement `Error` method of `error` interface.
