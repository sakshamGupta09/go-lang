...
- A data format computers use to communicate with each other. Works with JavaScript, Go, Python, Java, etc.
- We need to convert Go data types to JSON and JSON to Go data structures.

## Converting structs to JSON (Marshalling)

- We need to convert our `struct` to `json`.
- One option is that we use the exact same field names in `json` as we have them in `struct`. Sometimes, we do need that our `json` field names are different than `struct` field names.
- For every `struct` field, we need to define corresponsing JSON field in backticks.
- This is useful because our struct might be public and so we need to make the fields capitalCase. But in JSON, we might be following different naming conventions. Also, BE might be expecting some different schema than what we have defined.
...
- All fields in the struct that have falsy values like 0 or empty string or false will not be part of `json` representation.
...
- Convert `JSON` to structs. Let's say our BE is sending `JSON` response and we need to convert it into a struct.
...
- We can use a map.
- Contents of a JSON can be mapped to a struct (if structure is known) or to a map also.

```go
func main() {
	jsonData := `{"name": "John", "age": 28, "address": {"city": "City 1", "state": "State 1"}}`
