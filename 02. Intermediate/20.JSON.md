...
- We need to convert Go data structures to JSON and JSON to Go data structures.

## Converting structs to JSON (Marshalling)

- For every `struct` field, we need to define corresponsing JSON field in backticks.
- This is useful because our struct might be public and so we need to make the fields capitalCase. But in JSON, we might be following different naming conventions. Also, BE might eb expecting some different schema that we have defined.
