...
- Flexible arrays.
- Can grow or shrink their size during program execution.

## Internal Structure

- A slice is essentially a struct with three fields:

```go
type slice struct {
    ptr *T   // pointer to the underlying array
    len int  // current number of elements
    cap int  // maximum capacity from ptr
}
```
...
- This statement is not true. In Go, everything is passed by value.
- A slice internally is a struct. So, Go does create a copy of this slice but internally both are pointing to same underlying array.
...
- We can use traditional for loop from 0 to n-1.
...
- Both are referring to different slices. Updating one doesn't affects the other.
