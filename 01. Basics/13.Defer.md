...
- We have a function in which some instructions are written. The instructions are executed in sequential way. First then second and so on.
- Postpones the execution of a function untill surrouding function returns or finishes execution or panics.
- It’s mainly used for cleanup and resource management.
- Without defer, you'd need cleanup code before every return statement.
- Sometimes after performing a long task we forget to write cleanup in the end. With defer, we can write cleanup logic together wth the resource consuming activity.
