...
- It automatically adds date and time to our logged information.
- Methods like `printf`, `println` and `print` exists on `log` package. Automatically adds a new line charachter.
...
- Our application in production will be producing a ton of logs. It's important to differentiate.
...
```go
func main() {
	infoLogger := log.New(os.Stdout, "INFO:", log.Ldate|log.Ltime|log.Llongfile|log.Lshortfile)
...
func main() {
	...
	var infoLogger = log.New(file, "INFO: ", log.Ldate|log.Ltime|log.Llongfile|log.Lshortfile)

	infoLogger.Println("THIS IS A SAMPLE LOG")
...
- Logrus and Zap are third party Go libraries which makes logging easier.
